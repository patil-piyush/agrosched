      ...
      <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full px-6 py-12">
        <motion.div initial={{ opacity:0, y:20 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ duration:0.5 }}>
          <ModuleHeader module={6} title="Branch & Bound"
            subtitle="Systematic enumeration with intelligent pruning via upper-bound estimates. Explores far fewer nodes than brute force."
            timeComplexity="O(2ⁿ) worst · much better avg" spaceComplexity="O(2ⁿ) worst" />
        ...
        <motion.div initial={{ opacity:0 }} animate={inView ? { opacity:1 } : {}} transition={{ delay:0.2 }} className="flex-1 flex flex-col gap-4">
            ...
            <>
              ...
              <div className="algo-card">
                <div className="flex items-start justify-between mb-3 flex-wrap gap-3">
                  <div>
                    <h3 className="font-bold text-textDark">Knapsack B&B — {strat} Strategy</h3>
                    <p className="text-xs text-muted mt-0.5">
                      {strat==='FIFO' && 'Queue (BFS) — explores level by level'}
                      {strat==='LIFO' && 'Stack (DFS) — explores depth first'}
                      {strat==='LC'   && 'Priority Queue — always explores highest upper-bound node first (LC wins!)'}
                    </p>
                  </div>
                  <div className="flex gap-2 text-center">
                    <div className="px-3 py-2 rounded-lg" style={{ background: S_COLORS[strat]+'22', border:`1px solid ${S_COLORS[strat]}55` }}>
                    ...
                    <div className="px-3 py-2 rounded-lg bg-green-50">
                      ...
                      <div className="text-xs text-muted">optimal value</div>
                    </div>
                  </div>
                </div>

                <TreeVisualizer trace={stratData?.trace||[]} currentStep={bnbPlayer.currentStep} labelKey="idx" valueKey="value" maxNodes={40} />
                <div className="mt-3"><StepController player={bnbPlayer} label={`B&B ${strat}`} /></div>
              </div>

              {/* 3-way comparison */}
              <div className="algo-card">
                <h3 className="font-bold text-textDark mb-3">Strategy Comparison</h3>
                  ...
                  {STRATEGIES.map(s => {
                    ...
                    return (
                      <div key={s} className={`p-3 rounded-xl text-center border-2 ${isWinner?'border-green-400 bg-green-50':'border-gray-200 bg-gray-50'}`}>
                        ...
                        {isWinner && <div className="mt-1 text-xs font-bold text-green-700">🏆 Most efficient</div>}
                        {/* Bar */}
                        <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-700"
                            style={{ width:`${(d?.nodes/Math.max(bnbResult.FIFO.nodes,bnbResult.LIFO.nodes,bnbResult.LC.nodes))*100}%`, background:S_COLORS[s] }} />
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="mt-3 p-3 rounded-xl bg-green-50 text-sm text-center">
                  ...
                  <span className="text-green-700">LC wins by exploring {Math.round((1-bnbResult.LC.nodes/bnbResult.FIFO.nodes)*100)}% fewer nodes than FIFO.</span>
                </div>
              </div>
            </>
          )}
          ...
          {tab === 1 && (
            <div className="algo-card">
              <h3 className="font-bold text-textDark mb-1">Branch & Bound TSP — Plot Inspection Tour</h3>
              <p className="text-xs text-muted mb-4">Farmer visits all plots each week. B&B finds shortest route via LC with reduced-cost-matrix lower bounds.</p>

              {/* Tour map SVG */}
              <div className="relative bg-green-50 rounded-xl overflow-hidden mb-4" style={{ height: 320 }}>
                <svg viewBox="0 0 640 320" style={{ width:'100%', height:'100%' }}>
                  {/* Grid lines */}
                  {[1,2,3,4,5].map(i=><line key={i} x1={i*100} y1={0} x2={i*100} y2={320} stroke="#e5e7eb" strokeWidth={0.5}/>)}
                  {[1,2].map(i=><line key={i} x1={0} y1={i*107} x2={640} y2={i*107} stroke="#e5e7eb" strokeWidth={0.5}/>)}
                  ...
                  {tspPlayer.isDone && tspResult.tourCoords.length > 1 && (
                    tspResult.tourCoords.slice(0, -1).map((from, i) => {
                      ...
                      return (
                        <motion.line key={i}
                          x1={from.x * 1.1} y1={from.y * 1.1} x2={to.x * 1.1} y2={to.y * 1.1}
                  ...
                  {!tspPlayer.isDone && tspPlayer.currentStep >= 0 && (() => {
                    ...
                    return frame.tour.slice(0,-1).map((label, i) => {
                      const from = farm.plotCoordinates.find(p=>p.label===frame.tour[i]);
                      const to   = farm.plotCoordinates.find(p=>p.label===frame.tour[i+1]);
                      if (!from||!to) return null;
                      return (
                        <line key={i}
                          x1={from.x*1.1} y1={from.y*1.1} x2={to.x*1.1} y2={to.y*1.1}
                          stroke={frame.status==='pruned'?'#EF5350':'#F4A261'} strokeWidth={2}
                          strokeDasharray={frame.status==='pruned'?'6,4':''}
                        />
                      );
                    });
                  })()}
                  ...
                  {/* Nodes */}
                  {farm.plotCoordinates.map((p, i) => {
                    const isHome = p.id === 'FARM';
                    const inTour  = tspPlayer.isDone && tspResult.tour.includes(p.label);
                    return (
                      <g key={p.id}>
                        <circle cx={p.x*1.1} cy={p.y*1.1} r={isHome?14:12}
                          fill={isHome?'#2D6A4F':inTour?'#52B788':'#CFD8DC'}
                          stroke={isHome?'#1B5E3B':'#2D6A4F'} strokeWidth={2} />
                        <text x={p.x*1.1} y={p.y*1.1+1} textAnchor="middle" dominantBaseline="middle"
                          fontSize={isHome?8:9} fontWeight="700" fill="#fff" fontFamily="Inter">
                          {isHome?'🏠':p.id}
                        </text>
                        <text x={p.x*1.1} y={p.y*1.1+20} textAnchor="middle"
                          fontSize={8} fill="#6B7B6E">{p.label}</text>
                      </g>
                    );
                  })}
                </svg>
              ...
              {tspPlayer.isDone && (
                <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} className="mt-3 p-4 rounded-xl bg-green-50 border border-green-200">
                  <div className="font-bold text-green-800 mb-1">✓ Optimal Inspection Tour Found</div>
                  <div className="text-sm text-green-700 font-mono mb-1">
                    {tspResult.tour.join(' → ')}
                  </div>
                  <div className="flex gap-4 text-sm flex-wrap">
                    <span><strong>Distance:</strong> {tspResult.cost} units</span>
